...
import { Plus, Trash2, ArrowLeft, Save, Eye, Loader2, CheckCircle2, Circle, AlertCircle, ChevronUp, ChevronDown, Palette, Calendar as CalendarIcon, List, CheckSquare, Type, Hash, Languages, Upload, X, GitBranch } from 'lucide-react';
...
export default function EditSurvey({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function checkAuthAndFetch() {
      ...
      try {
        ...
        setThemeColor(survey.theme_color || '#0ea5e9');
        setLogoUrl(survey.logo_url || null);
        setQuestions(questionsData.map(q => ({
          ...
          logic: q.logic ? {
            triggerQuestionId: questionsData.find(pd => pd.order === q.logic.condition.questionOrder)?.id || '',
            operator: q.logic.condition.operator,
            value: q.logic.condition.value
          } : undefined
        })));
      } catch (err) {
  ...
  const addQuestion = () => {
    setQuestions([...questions, { id: Math.random().toString(), label: '', hint: '', type: 'text', options: [], required: false, isNew: true }]);
  };
  ...
  const removeQuestion = (id: string, isNew?: boolean) => {
    if (questions.length > 1) {
      ...
      if (!isNew) {
        setRemovedQuestionIds([...removedQuestionIds, id]);
      }
  ...
  const saveSurvey = async () => {
    ...
    try {
      ...
      const { error: surveyError } = await supabase
        .from('surveys')
        .update({ title, description, is_active: isActive, theme_color: themeColor, logo_url: logoUrl })
        .eq('id', surveyId);
      ...
      // - Upsert existing questions and Insert new ones separately to avoid heterogeneous array issues
      const existingQuestions = validQuestions
        .filter(q => !q.isNew)
        .map(q => {
          let dbLogic = null;
          ...
          return {
            ...
            order: validQuestions.indexOf(q),
            logic: dbLogic
          };
        });

      const newQuestions = validQuestions
        .filter(q => q.isNew)
        .map(q => {
          let dbLogic = null;
          if (q.logic && q.logic.triggerQuestionId) {
      ...
      toast.success('บันทึกการเปลี่ยนแปลงเรียบร้อยแล้ว', { id: toastId });
      router.push('/');
      router.refresh();
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
            {isPreview ? 'ตัวอย่างแบบสอบถาม' : 'แก้ไขแบบสอบถาม'}
          </h1>
        </div>
        <Button 
          variant="outline" 
        ...
        <div className="space-y-6">
          <Card className="border-t-4 shadow-lg dark:bg-slate-900" style={{ borderTopColor: themeColor }}>
            <CardHeader>
              ...
              <CardTitle className="text-2xl dark:text-slate-100">{title || 'ชื่อแบบสอบถาม'}</CardTitle>
              {description && <CardDescription className="text-base dark:text-slate-400">{description}</CardDescription>}
            </CardHeader>
          </Card>
          ...
          {questions.filter(q => q.label.trim() !== '').map((q, i) => (
            <Card key={q.id} className="shadow-sm border-none bg-white dark:bg-slate-900">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg flex items-start gap-2 dark:text-slate-100">
              ...
              <CardContent className="pt-2">
                {q.type === 'text' && (
                  <Input placeholder="คำตอบของคุณ..." disabled className="bg-slate-50/50 dark:bg-slate-950 dark:border-slate-800" />
                )}
                {q.type === 'choice' && (
                  <div className="space-y-2">
                    {q.options.map((opt, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950">
                  ...
                  <div className="flex justify-between max-w-sm mx-auto py-4">
                    {[1, 2, 3, 4, 5].map((num) => (
                      <div key={num} className="flex flex-col items-center gap-2">
                        <div className="w-10 h-10 rounded-full border-2 border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-400 font-bold">
                          {num}
                        </div>
                      </div>
                    ))}
          ...
          <div className="p-4 rounded-xl text-center text-sm italic border-2 border-dashed bg-slate-50 dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800">
            * นี่คือหน้าตัวอย่าง (Preview)
          </div>
        </div>
      ) : (
        <>
          <Card className="shadow-md border-primary/10 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
            <div className="h-2 w-full" style={{ backgroundColor: themeColor }} />
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg dark:text-slate-100">ข้อมูลพื้นฐาน</CardTitle>
                  ...
                  <div className="flex gap-1.5 items-center">
                    {['#0ea5e9', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#64748b'].map(color => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-full border-2 transition-all ${themeColor === color ? 'border-slate-900 dark:border-white scale-110' : 'border-transparent hover:scale-105'}`}
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="dark:text-slate-200">โลโก้คลินิก (Optional)</Label>
                <div className="flex items-center gap-4">
                  {logoUrl ? (
                    <div className="relative group">
                      <div className="w-24 h-24 border dark:border-slate-700 rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
              ...
              <div className="space-y-2">
                <Label htmlFor="survey-title" className="dark:text-slate-200">ชื่อแบบสอบถาม</Label>
                <Input 
                  id="survey-title"
              ...
              <div className="space-y-2">
                <Label htmlFor="survey-desc" className="dark:text-slate-200">รายละเอียด (ไม่บังคับ)</Label>
                <Input 
                  id="survey-desc"
               ...
               <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700">
                 ...
                 />
                 <Label htmlFor="survey-active" className="cursor-pointer font-bold select-none flex-1 dark:text-slate-200">
                   {isActive ? (
                     ...
                     <span className="flex items-center text-slate-500 dark:text-slate-400 gap-1.5">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="font-bold dark:border-slate-700 dark:text-slate-300">{questions.length} ข้อ</Badge>
            </div>
            ...
            <div className="space-y-4">
              <AnimatePresence initial={false}>
                {questions.map((q, index) => (
                  ...
                  >
                    <Card className="relative group shadow-sm border-slate-200 dark:border-slate-800 overflow-hidden hover:border-slate-300 dark:hover:border-slate-700 transition-all dark:bg-slate-900">
                      ...
                      <CardContent className="pt-6 space-y-4">
                          ...
                          <div className="flex-1 space-y-4">
                            <div className="space-y-2">
                              ...
                              <Input 
                                ...
                                className="h-11 font-medium dark:bg-slate-950 dark:border-slate-800"
                              />
                            </div>
                            <div className="space-y-2">
                              <Label className="text-slate-500 text-xs dark:text-slate-400">คำอธิบายเพิ่มเติม (Optional)</Label>
                              <Input 
                                placeholder="เช่น ระบุเป็นตัวเลขเท่านั้น" 
                                value={q.hint}
                                onChange={(e) => updateQuestion(q.id, { hint: e.target.value })}
                                className="h-9 text-sm bg-slate-50/50 dark:bg-slate-950 dark:border-slate-800"
                              />
                            </div>
                          </div>
                          <div className="w-full sm:w-48 space-y-2">
                            <Label className="dark:text-slate-200">ประเภทคำถาม</Label>
                            <Select
                              value={q.type}
                              onValueChange={(value: QuestionType) => 
                                updateQuestion(q.id, { 
                                  type: value, 
                                  options: value === 'choice' ? (q.options.length > 0 ? q.options : ['']) : [] 
                                })
                              }
                            >
                              <SelectTrigger className="w-full h-11 bg-white dark:bg-slate-950 dark:border-slate-800">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <Label htmlFor={`req-${q.id}`} className="text-sm font-medium cursor-pointer select-none dark:text-slate-300">
                          ...
                          <Button 
                            ...
                            onClick={() => removeQuestion(q.id, q.isNew)}
                          >
                          ...
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                             {!q.logic ? (
                               <Button
                                 ...
                                 className="text-slate-500 hover:text-primary gap-2 dark:text-slate-400"
                               >
                               ...
                               <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 space-y-3">
                                 ...
                                 <div className="flex flex-col sm:flex-row gap-3">
                                   <div className="flex-1">
                                      ...
                                      >
                                        <SelectTrigger className="h-9 bg-white dark:bg-slate-950 dark:border-slate-700 text-xs">
                                   ...
                                   <div className="w-24 shrink-0 flex items-center justify-center text-xs text-slate-500 dark:text-slate-400 font-medium">
                                      เท่ากับ
                                   </div>
                                   <div className="flex-1">
                                      <Select
                          ...
                          <div className="space-y-3 pl-4 border-l-2 border-slate-200 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-800/30 p-4 rounded-r-lg">
                            <div className="flex items-center justify-between">
                              ...
                              {(q.type === 'choice' || q.type === 'checkboxes') && (
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  <Label htmlFor={`other-${q.id}`} className="text-xs cursor-pointer dark:text-slate-300">เพิ่มตัวเลือก "อื่นๆ"</Label>
                                </div>
                              )}
                            </div>
                            <AnimatePresence initial={false}>
                              {q.options.map((opt, optIdx) => (
                                ...
                                >
                                  <div className="flex items-center justify-center w-10 h-10 shrink-0">
                                    {q.type === 'choice' || q.type === 'dropdown' ? <Circle className="h-4 w-4 text-slate-300" /> : <CheckSquare className="h-4 w-4 text-slate-300" />}
                                  </div>
                                  <Input 
                                    placeholder={`ตัวเลือกที่ ${optIdx + 1}`} 
                                    value={opt}
                                    onChange={(e) => updateOption(q.id, optIdx, e.target.value)}
                                    className="bg-white dark:bg-slate-950 dark:border-slate-700 h-10"
                                  />
                            ...
                            {q.allowOther && (
                              <div className="flex gap-2 opacity-60">
                                <div className="flex items-center justify-center w-10 h-10 shrink-0">
                                  {q.type === 'choice' ? <Circle className="h-4 w-4 text-slate-300" /> : <CheckSquare className="h-4 w-4 text-slate-300" />}
                                </div>
                                <Input value='อื่นๆ...' disabled className="bg-slate-100/50 dark:bg-slate-800/50 h-10" />
                                <div className="w-10" />
                              </div>
                            )}
                            <Button variant="outline" size="sm" className="text-primary border-primary/20 hover:bg-primary/5 dark:hover:bg-primary/20 h-10 w-full dashed border-2 border-dashed" onClick={() => addOption(q.id)}>
            ...
            <Button 
              type="button" 
              variant="outline" 
              className="w-full py-8 border-dashed border-2 hover:bg-slate-50 hover:border-primary/50 dark:hover:bg-slate-900 transition-all group"
              onClick={addQuestion}
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="font-bold text-slate-500 group-hover:text-primary dark:text-slate-400">เพิ่มคำถามใหม่</span>
              </div>
            </Button>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            <Button variant="outline" className="flex-1 h-12 border-primary text-primary hover:bg-primary/5 font-bold" onClick={addQuestion}>
              <Plus className="h-4 w-4 mr-2" /> เพิ่มคำถามใหม่
            </Button>
            <Button className="flex-1 h-12 font-bold shadow-lg shadow-primary/20" onClick={saveSurvey} disabled={isSaving}>
